...
- One file per page: `src/pages/<id>.tsx`. Export `page = { id: '<id>', label: '<Tab label>' }`
...
- Reuse existing page structure before inventing layout: `src/app/list-page-layout.tsx`
  and `src/pages/customers.tsx` show the list-page pattern (title, description, quick
...
- Typecheck with `node_modules/.bin/tsc --noEmit -p tsconfig.app.json`. Six diagnostics are
  known baseline (`texo-icons.tsx`, `texo-theme-provider.tsx`, `app.tsx` replaceAll,
  `custom-components-page.tsx` x2, `theme-controls.tsx`); anything else is yours.

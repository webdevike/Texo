...
const componentClasses = { card: 'texo-card', input: 'texo-input', checkboxInput: 'texo-checkbox-input', popover: 'texo-popover' };
...
export function TexoThemeProvider({ children, initial, onChange }: {
  children: ReactNode;
  /** Persisted config to start from; a preset name or a full config. Absent = first preset. */
  initial?: { config: TexoThemeConfig; preset: string };
  /** Fires on every committed change so a host can persist it. */
  ...
  const [seeded, setSeeded] = useState(initial !== undefined);
  // Adopt a persisted config that arrives after mount (the host fetches it asynchronously).
  if (!seeded && initial) {
    setHistory({ future: [], past: [], present: initial.config, preset: initial.preset });
    setSeeded(true);
  }
  useEffect(() => { onChange?.(history.present, history.preset); }, [history.present, history.preset]);
  ...
  const themeOverride = useMemo<MantineThemeOverride>(() => ({
    ...
    primaryShade: history.present.primaryShade as { light: number; dark: number } as never,
